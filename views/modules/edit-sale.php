...
function json_decod($json)
{
$comment = false;
$out = '$x=';
for ($i=0; $i<strlen($json); $i++) { if (!$comment) { if (($json[$i] == '{') || ($json[$i] == '[')) $out .= ' array('; else if (($json[$i] == '}') || ($json[$i] == ']')) $out .= ')'; else if ($json[$i] == ':') $out .= '=>';
else
$out .= $json[$i];
}
else
...
// $text=$sale["products"];
//                         $productList = json_decode($text);
// echo $productList;
//$productList = new RecursiveIteratorIterator( new RecursiveArrayIterator(json_decode($sale["products"], TRUE)), RecursiveIteratorIterator::SELF_FIRST);
  $productList = json_decode($sale["products"], true);
                        ...
                        foreach ($productList as $key => $value) {
                         ...
                         $answer = ControllerProducts::ctrShowproducts($item, $valueProduct, null);
